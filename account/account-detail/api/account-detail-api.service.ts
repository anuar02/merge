...
import { FilterPostsPayload, Post } from '../account';
...
export class AccountDetailApiService {
    ...
    getGeneralInfo(id: string): Observable<AccountEntity> {
        return this.http.get<AccountEntity>(`resources/account/${id}`).pipe(
            switchMap((group) => {
                if (!group.photo) {
                    return of({
                        ...group,
    ...
    getProfile(id: string): Observable<AccountProfileModel> {
        return this.http.get<AccountProfileModel>(`resources/account/${id}/profile`);
    }
